...
function KenoBoard() {
  ...
  const addPicked = (e, val) => {
    ...
    if (val in picked) {
      ...
      setPicked(picked.filter((num) => num !== val));
      e.currentTarget.classList.remove(" picked");
    }
  
    if (picked.length < pickLimit && !picked.includes(val)) {
      ...
      // e.currentTarget.className += " pick";
    ...
    return (
      <div className="options">
        ...
        <button
          ...
          className="bg-slate-400 hover:bg-slate-300 cursor-pointer rounded-md px-4 py-2"
          onClick={finGetWin}
        >
          Submit
        </button>
                ...
                {row.map(col =>
                  (
                    <div
                      ...
                      className={picked.indexOf(col) !== -1 ? "col pick" : "col"}
                      value={col}
                    >
                      {renderNum(col)}
                    </div>
  ...
  return (
    <>
      <h1>Chance Keno</h1>
      <p>
        Pick up to {pickLimit} Keno spots or choose Quik Pik (defaults to{" "}
        {pickLimit} numbers) by clicking on the number in the Keno card.
      </p>
      {/* <p>
